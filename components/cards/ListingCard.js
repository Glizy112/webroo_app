...
const ListingCard = ({ element, index, width, height, cardHeight, list, handleClick, detailFormData }) => {
  const cardBGImage =
  ...
  console.log("Detail Data", detailFormData);
    ...
    return (
      <Link
        ...
        {{pathname: 
            ...
            query: {data: detailFormData}
        }}
        as={`/facility/${list?.id}/${list?.publicName}`}
      >
        <div className="card-container1" style={{ width: width ? width : 375, height: cardHeight ? cardHeight : 'auto'}} onClick={handleClick}>
